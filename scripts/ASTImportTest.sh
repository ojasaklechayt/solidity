...
function test_evmjson_via_ir_and_yul_import_export
{
    ...
    do
      ...
      then
          # if there was a difference, we will copy the used files of that
          # test-case to /tmp/import-export/ to allow the investigation of those.
          name=$(basename "${sol_file}")
          mkdir -p "/tmp/import-export/$name"

          printError "ERROR: diff failed ${file} ${yul}"
          printError "$(pwd)"
          printError "${sol_file} ${input_files[*]}"
          echo "- ${type} @ ${sol_file}" >> /tmp/import-export/all_errors.txt

          cp diff_error "/tmp/import-export/$name"
          cp -r input "/tmp/import-export/$name"
          cp -r yul "/tmp/import-export/$name"
          cp -r sol "/tmp/import-export/$name"
