...
Json::Value CompilerStack::assemblyJSON(string const& _contractName) const
{
	...
	if (currentContract.evmAssembly)
	{
		if (!m_evmAssemblyStack)
		{
			...
			sourceList.reserve(m_sourceOrder.size());
			for (auto const& source: m_sourceOrder)
				sourceList.emplace_back(source->charStream->name());
			if (std::find(sourceList.begin(), sourceList.end(), CompilerContext::yulUtilityFileName()) == sourceList.end())
				sourceList.emplace_back(CompilerContext::yulUtilityFileName());
			currentContract.evmAssembly->setSourceList(sourceList);
		}
